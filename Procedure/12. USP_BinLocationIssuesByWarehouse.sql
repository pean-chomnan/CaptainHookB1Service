 CALL "USP_BinLocationIssuesByWarehouse"('1RMSNFRWH','03.HKT05');

ALTER PROCEDURE "USP_BinLocationIssuesByWarehouse" (IN ItemCode nvarchar(100), IN WhsCode nvarchar(200)) 
AS 
	BEGIN
		DV20210722100557X67N_2VA1CDO_7A1=(
			...
			WHERE T1."AbsEntry" >= (0) AND (T2."AbsEntry" IS NOT NULL) 
				AND T0."ItemCode" IN ((SELECT U0."ItemCode" FROM CAPTAINHOOK_PRD."OITM" U0 INNER JOIN CAPTAINHOOK_PRD."OITB" U1 ON U0."ItmsGrpCod" = U1."ItmsGrpCod" WHERE U0."ItemCode" IS NOT NULL AND U0."ItemCode" >= (:ItemCode) 
					AND U0."ItemCode" <= (:ItemCode))) AND  T1."WhsCode"=WhsCode -- AND T4."DistNumber" >= (:BatchCode) AND T4."DistNumber" <= (:BatchCode) 
		...
		WHERE T1."AbsEntry" >= (0) AND (T2."AbsEntry" IS NOT NULL) 
			...
			AND T0."WhsCode"=WhsCode --AND T4."DistNumber" >= (:BatchCode) AND T4."DistNumber" <= (:BatchCode) 
		...
		WHERE T0."IBQOnhandQty" > T0."OnHandQty"
		 AND  T0."WhsCode"=WhsCode; -- AND T0."BTNDistNumber" >= (:BatchCode) AND T0."BTNDistNumber" <= (:BatchCode);
